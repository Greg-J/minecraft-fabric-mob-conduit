...
public final class FrameShape {
	 ...
	 * block it stands on.
	 */
	public static int count(World world, ConduitPos centre, Material frameBlock) {
		int centreX = centre.x();
		int centreY = centre.y();
		int centreZ = centre.z();
		int found = 0;

		for (int i = 0; i < OFFSETS.length; i += 3) {
			if (world.getBlockAt(centreX + OFFSETS[i], centreY + OFFSETS[i + 1], centreZ + OFFSETS[i + 2]).getType() == frameBlock) {
				found++;
			}
		}
